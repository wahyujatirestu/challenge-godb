...
func (r *serviceRepo) GetById(id int) (entity.Service, error) {
	...
	err := r.db.QueryRow("SELECT service_id, service_name, unit, price, updated_at FROM service WHERE service_id = $1", &s.Service_Id, &s.Service_Name, &s.Unit, &s.Price, &s.CreatedAt, &s.UpdatedAt)
