...
func (r *orderRepository) Create(order entity.Order, details []entity.OrderDetail) error {
	...
	_, err = tx.Exec(`INSERT INTO "order"(order_id, customer_id, order_date, received_by) VALUES($1, $2, $3, $4)`, order.Order_Id, order.Customer_Id, order.Order_Date, order.Received_By)
...
func (r *orderRepository) GetAll() ([]entity.Order, error) {
	...
	for rows.Next() {
		var o entity.Order
		err := rows.Scan(&o.Customer_Id, &o.Customer_Id, &o.Order_Date, &o.Completion_Date, &o.Received_By, &o.Created_At, &o.Updated_At)
		if err != nil {
			return nil, err
		}
...
func (r *orderRepository) FindById(orderId int) (*entity.Order, []entity.OrderDetail, error) {
	...
	err := r.db.QueryRow(`SELECT order_id, customer_id, order_date, completion_date, received_by FROM "order" WHERE order_id=$1`, orderId).Scan(&o.Order_Id, &o.Customer_Id, &o.Order_Date, &o.Completion_Date, &o.Received_By)
	...
	rows, err := r.db.Query(`SELECT SELECT order_detail_id, order_id, service_id, qty FROM order_detail WHERE order_id=$1`, orderId)
...
func (r *orderRepository) IsOrderIdExist(orderId int) bool {
	var exist bool
	r.db.QueryRow(`SELECT EXIST(SELECT 1 FROM "order" WHERE order_id=$1 )`, orderId).Scan(&exist)
	return exist
}
...
func (r *orderRepository) IsCustomerIdExist(customerId int) bool {
	var exist bool
	r.db.QueryRow(`SELECT EXIST(SELECT 1 FROM customer WHERE customer_id=$1)`, customerId).Scan(&exist)
	return exist	
}